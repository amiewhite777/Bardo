 ...
 */
function generateRealisticSession(sessionId: string): QuizSession {
 ...
 */
export function runRealisticSimulation(sessionCount: number = 10000): RealisticSimResults {
  console.log(`\n🎲 Generating ${sessionCount} realistic quiz sessions (pure random)...\n`);

  const results: RealisticSimResults = {
 ...
 */
export function printComparisonReport(results: RealisticSimResults): void {
  ...
  for (const realm of realms) {
    ...
    console.log(
      `${status} ${realm.padEnd(12)} | ${predicted.toFixed(1).padStart(8)}% | ${actual.toFixed(1).padStart(8)}% | ${(diff >= 0 ? '+' : '')}${diff.toFixed(1).padStart(5)}%`
    );
  ...
  for (const [form, targetPct] of allForms) {
    ...
    console.log(
      `${rank.toString().padStart(2)}   | ${form.padEnd(26)} | ${predicted.toFixed(2).padStart(8)}% | ${actual.toFixed(2).padStart(8)}% | ${(diff >= 0 ? '+' : '')}${diff.toFixed(2).padStart(6)}% | ${count.toString().padStart(6)} | ${status}`
    );
