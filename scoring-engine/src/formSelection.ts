 ...
 */
export function calculateSubcategory(state: ScoringState, responses: QuizResponse[]): void {
  if (!state.primaryRealm) return;
 ...
 */
function getSubcategoryFromTopology(realm: Realm, responses: QuizResponse[]): string {
  ...
  const topologyMap: Record<Realm, Record<string, string>> = {
    Hell: {
 ...
 */
export function determineForm(state: ScoringState, responses: QuizResponse[]): void {
  if (!state.primaryRealm || !state.subcategory) return;
 ...
 */
function scoreFormsInSubcategory(
  realm: Realm,
  subcategory: string,
  candidates: string[],
...
function scoreHellForms(subcategory: string, scores: Record<string, number>, responses: QuizResponse[], state: ScoringState): void {
  if (subcategory === 'Burning') {
    const q7 = getResponse(responses, 'Q7');
    const q47 = getResponse(responses, 'Q47');

    if (q7 === 'a' || q47 === 'a') scores['The Inferno'] += 3; // Undifferentiated rage
    if (q7 === 'a') scores['The Scorcher'] += 2; // Targeted (needs more sophistication)

    const q34 = getResponse(responses, 'Q34');
    if (q34 === 'a') scores['The Spreader'] += 3; // Weaponize information
  } else if (subcategory === 'Freezing') {
    const q8 = getResponse(responses, 'Q8');
    const q37 = getResponse(responses, 'Q37');

    if (q8 === 'b' || q37 === 'a') scores['The Glacier'] += 3; // Complete shutdown
    if (getResponse(responses, 'Q7') === 'b') scores['The Bitter'] += 3; // Cold calculation
    if (state.shadowFlags.includes('DENIED_ANGER')) scores['The Numb'] += 3; // Suppressed anger
  } else if (subcategory === 'Crushing') {
    const q8 = getResponse(responses, 'Q8');
    const q19 = getResponse(responses, 'Q19');

    if (q8 === 'c' || q19 === 'a') scores['The Judge'] += 3; // Correcting/judging cruelty
    const q9 = getResponse(responses, 'Q9');
    if (q9 === 'b') scores['The Executioner'] += 3; // Being lied to triggers punishment
  } else if (subcategory === 'Repetition') {
    const q8 = getResponse(responses, 'Q8');
    const q46 = getResponse(responses, 'Q46');

    if (q8 === 'd' || q46 === 'a') scores['The Ruminant'] += 3; // Replaying conversations
...
function scoreHungryGhostForms(subcategory: string, scores: Record<string, number>, responses: QuizResponse[], state: ScoringState): void {
  if (subcategory === 'Mouth') {
    const q11 = getResponse(responses, 'Q11');
    const q27 = getResponse(responses, 'Q27');

    if (q11 === 'a' || (q27 === 'a' || q27 === 'b')) scores['The Glutton'] += 3; // Urgent wanting/high sense desire
    if (q11 === 'b' || getResponse(responses, 'Q12') === 'b') scores['The Connoisseur'] += 3; // Specific/curated
    if (getResponse(responses, 'Q14') === 'a') scores['The Addict'] += 3; // Controls me
  } else if (subcategory === 'Status') {
    const q12 = getResponse(responses, 'Q12');
    const q33 = getResponse(responses, 'Q33');

    if (q12 === 'b') scores['The Collector'] += 3; // Curating
    if (getResponse(responses, 'Q13') === 'b' || q33 === 'b') scores['The Climber'] += 3; // Status markers/transactional
    if (q12 === 'b' && getResponse(responses, 'Q38') === 'b') scores['The Influencer'] += 3; // Curating + seeking node
  } else if (subcategory === 'Void') {
    const q12 = getResponse(responses, 'Q12');
    const q11 = getResponse(responses, 'Q11');

    if (q12 === 'c') scores['The Browser'] += 3; // Browse endlessly
    if (q11 === 'c' || getResponse(responses, 'Q14') === 'c') scores['The Emptiness'] += 3; // Can't name want/vague
  } else if (subcategory === 'Theft') {
    const q11 = getResponse(responses, 'Q11');
    const q36 = getResponse(responses, 'Q36');

    if (q11 === 'd') scores['The Coveter'] += 3; // Want what others have
    if (getResponse(responses, 'Q34') === 'b' || q36 === 'b') scores['The Taker'] += 3; // Hoard/calculate loss
  }
}

function scoreAnimalForms(subcategory: string, scores: Record<string, number>, responses: QuizResponse[], state: ScoringState): void {
  if (subcategory === 'Prey') {
    const q3 = getResponse(responses, 'Q3');
    const q30 = getResponse(responses, 'Q30');

    if ((q3 === 'a' || q30 === 'a')) scores['The Rabbit'] += 3; // Hypervigilant/extremely agitated
    if (q30 === 'b') scores['The Deer'] += 3; // Restless
    if (getResponse(responses, 'Q35') === 'c') scores['The Mouse'] += 3; // Undefined network/hiding
  } else if (subcategory === 'Predator') {
    const q34 = getResponse(responses, 'Q34');

    if (q34 === 'a') scores['The Shark'] += 2; // Weaponize info (overlaps with Hell)
    scores['The Wolf'] += 1; // Default for pack behavior
    if (getResponse(responses, 'Q47') === 'c') scores['The Snake'] += 3; // Move on immediately
  } else if (subcategory === 'Herd') {
    const q33 = getResponse(responses, 'Q33');
    const q41 = getResponse(responses, 'Q41');

    if (q33 === 'c' || q41 === 'c') scores['The Sheep'] += 3; // Merging/social proof
    if (getResponse(responses, 'Q35') === 'c') scores['The Cattle'] += 3; // Undefined network/routine
  } else if (subcategory === 'Torpor') {
    const q29 = getResponse(responses, 'Q29');
    const q15 = getResponse(responses, 'Q15');

    if (q29 === 'a' || q29 === 'b') scores['The Sloth'] += 3; // Very low energy
    if (q15 === 'c') scores['The Hibernator'] += 3; // Freeze response
  }
}

function scoreHumanForms(subcategory: string, scores: Record<string, number>, responses: QuizResponse[], state: ScoringState): void {
  if (subcategory === 'Seekers') {
    ...
    if (q13 === 'e') scores['The Creator'] += 3; // Meaningful goals
    if (q40 === 'd') scores['The Achiever'] += 3; // Genuinely different
    ...
    if (q4 === 'd') scores['The Wounded'] += 3; // Loss wound
    if (getResponse(responses, 'Q38') !== 'd') scores['The Frustrated'] += 2; // Trying but stuck
    if (q31 === 'a') scores['The Desperate'] += 3; // Deeply uncertain/crisis
  } else if (subcategory === 'Sleepers') {
    const q40 = getResponse(responses, 'Q40');
    const q27 = getResponse(responses, 'Q27');

    if (q27 === 'c' || q27 === 'd') scores['The Comfortable'] += 3; // Low hindrances
    if (getResponse(responses, 'Q12') === 'c') scores['The Distracted'] += 3; // Browse endlessly
  }
}

function scoreAsuraForms(subcategory: string, scores: Record<string, number>, responses: QuizResponse[], state: ScoringState): void {
  if (subcategory === 'Warriors') {
    const q13 = getResponse(responses, 'Q13');
    const q39 = getResponse(responses, 'Q39');

    if (q13 === 'h' || q39 === 'a') scores['The Conqueror'] += 3; // Victory/righteous cause
    if (getResponse(responses, 'Q7') === 'h' || getResponse(responses, 'Q47') === 'e') {
      scores['The Fighter'] += 3; // Anger/combat ready
    }
    if (getResponse(responses, 'Q34') === 'a') scores['The Underminer'] += 3; // Weaponize info covertly
  } else if (subcategory === 'Measurers') {
    const q12 = getResponse(responses, 'Q12');
    const q45 = getResponse(responses, 'Q45');

    if (q12 === 'h') scores['The Ranker'] += 3; // Track progress
    if (q45 === 'e') scores['The One-Upper'] += 3; // Immediate comparison
    if (getResponse(responses, 'Q22') === 'e') scores['The Subtle Brag'] += 3; // Achieving identity
  } else if (subcategory === 'Resenters') {
    const q4 = getResponse(responses, 'Q4');
    const q20 = getResponse(responses, 'Q20');

    if (q4 === 'e' || q20 === 'd') scores['The Victim'] += 3; // Inadequacy/lost-struggling
    ...
    if (q12 === 'h' && getResponse(responses, 'Q34') === 'b') scores['The Accumulator'] += 3; // Track + hoard
    if (q45 === 'e') scores['The Thief of Joy'] += 3; // Can't enjoy others' success
  }
}

function scoreDevaForms(subcategory: string, scores: Record<string, number>, responses: QuizResponse[], state: ScoringState): void {
  if (subcategory === 'Pleasure') {
    const q11 = getResponse(responses, 'Q11');
    const q44 = getResponse(responses, 'Q44');

    if (q11 === 'i' || q44 === 'f') scores['The Hedonist'] += 3; // Have what want/enjoy pleasure
    if (getResponse(responses, 'Q1') === 'f' || getResponse(responses, 'Q3') === 'f') {
      scores['The Protected'] += 3; // Birth privilege/assumed safety
    }
    if (getResponse(responses, 'Q9') === 'i') scores['The Entitled'] += 3; // Unnecessary problems trigger withdrawal
  } else if (subcategory === 'Accomplished') {
    const q16 = getResponse(responses, 'Q16');
    const q38 = getResponse(responses, 'Q38');

    if (q16 === 'f') scores['The Retired'] += 3; // Done enough self-work
    if (getResponse(responses, 'Q34') === 'f') scores['The Expert'] += 3; // Appreciate (domain mastery)
    if (q38 === 'f') scores['The Mentor'] += 3; // Stable hub
  } else if (subcategory === 'Spiritual') {
    const q10 = getResponse(responses, 'Q10');
    const q39 = getResponse(responses, 'Q39');

    if (q10 === 'i' || q39 === 'f') scores['The Guru'] += 3; // Transcended anger/complacent transcendence
    if (getResponse(responses, 'Q30') === 'e') scores['The Blissed'] += 3; // Very peaceful
    ...
    if (q31 === 'e' || q40 === 'f') scores['The Comfortable'] += 3; // Unquestioned/comfortable not transformed
