import { ScoringState, QuizResponse, Realm, ShadowFlagType } from './types.js';
import { getResponse } from './scorer.js';

/**
 * Detect all shadow flags based on response patterns
 */
export function detectShadowFlags(state: ScoringState, responses: QuizResponse[]): void {
  ...
  if (q19Response) {
    const projectionMap: Record<string, Realm> = {
      a: 'Hell',
      b: 'HungryGhost',
      c: 'Animal',
      d: 'Human',
 ...
 */
export function calculateShadowGap(state: ScoringState, responses: QuizResponse[]): void {
 ...
 */
export function applyShadowModifiers(state: ScoringState): void {
...
function getHighestRealmForProjection(state: ScoringState): Realm {
...
function isPositiveSelfImage(answer: string | undefined): boolean {
  // Q22: Noble story - d, e, f are more positive
  return answer === 'd' || answer === 'e' || answer === 'f';
}

function isNegativeCriticImage(answer: string | undefined): boolean {
  // Q20: Worst critic - a, b, c are more negative
  return answer === 'a' || answer === 'b' || answer === 'c';
}

function getSelfImageScore(answer: string | undefined): number {
  if (!answer) return 0;
...
function getCriticImageScore(answer: string | undefined): number {
  if (!answer) return 0;
...
function getAcknowledgedShadowScore(answer: string | undefined): number {
  if (!answer) return 0;
...
function getProjectionStrength(responses: QuizResponse[]): number {
 ...
 */
export function interpretShadowGap(shadowGap: number): string {
  if (shadowGap <= 10) {
