 ...
 */
export function recordUserResult(result) {
    ...
    for (const [form, targetPct] of Object.entries(FORM_POPULATIONS)) {
        ...
        if (Math.abs(relativeError) > 0.1) {
            ...
            if (Math.abs(newWeight - currentWeight) > 0.05) {
                ...
                console.log(`${direction} ${form.padEnd(30)} ` +
 ...
 */
export function getFormWeight(form) {
    const data = loadCalibrationData();
 ...
 */
export function getAllFormWeights() {
    const data = loadCalibrationData();
 ...
 */
export function printCalibrationStatus() {
    const data = loadCalibrationData();
    console.log('\n' + '='.repeat(80));
    ...
    if (data.totalSessions > 0) {
        ...
        for (const [form, count] of sortedForms) {
            ...
            console.log(`${form.padEnd(26)}| ${actualPct.toFixed(2).padStart(6)}% | ` +
    ...
    if (data.updateHistory.length > 0) {
        ...
        for (const update of data.updateHistory.slice(-5)) {
            ...
            console.log(`${date.padEnd(25)}| ${update.sessionCount.toString().padStart(8)} | ` +
