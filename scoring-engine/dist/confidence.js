...
export function calculateConfidence(state) {
    ...
    const consistencyPasses = (state.birthRealm === state.primaryRealm || // Stable trajectory
 ...
 */
export function getTrajectory(birthRealm, currentRealm, trajectoryScore) {
    if (!birthRealm || !currentRealm)
        return 'stable';
    ...
    if (trajectoryScore < -20)
        return 'crisis';
    // Compare heights
    if (currentHeight > birthHeight + 1)
        return 'growth';
    if (currentHeight < birthHeight - 1)
        return 'regression';
    return 'stable';
