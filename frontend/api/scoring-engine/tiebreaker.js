 ...
 */
export function breakTie(realm1, realm2, responses, state) {
 ...
 */
export function determinePrimaryRealm(state, responses) {
 ...
 */
function getSpeedRoundDominantRealm(state) {
    const realms = ['Hell', 'HungryGhost', 'Animal', 'Human', 'Asura', 'Deva'];
    let maxScore = -1;
    let maxRealm = 'Animal';
 ...
 */
export function calculateConsistency(state, responses) {
    ...
    if (state.consideredRealm && state.speedRoundRealm) {
        ...
        const realmOrder = ['Hell', 'HungryGhost', 'Animal', 'Human', 'Asura', 'Deva'];
 ...
 */
function scenariosMatch(selfReport, scenario, type) {
    if (type === 'anger') {
        // Q10 vs Q47
