 ...
 */
function generateRandomSession(sessionId, bias) {
    const responses = [];
    ...
    for (const question of quizData.questions) {
        ...
        if (bias) {
            ...
            const biasedOptions = options.filter((opt) => opt.scoring.some((s) => s.realm === bias));
 ...
 */
function generateWeightedSessions(count) {
    const sessions = [];
 ...
 * Run simulation
 */
export function runSimulation(sessionCount = 1000) {
    ...
    for (const realm of Object.keys(results.realmCounts)) {
...
export function printSimulationReport(results) {
    ...
    for (const realm of realms) {
        ...
        console.log(`${status} ${realm.padEnd(12)} | ${actual.toFixed(1)}%  | ${target.toFixed(1)}%  | ${diff >= 0 ? '+' : ''}${diff.toFixed(1)}%    | ${count}`);
    ...
    for (const [form, count] of forms) {
        ...
        const diff = actual - target;
        console.log(`${form.padEnd(24)}| ${count.toString().padStart(5)} | ${actual.toFixed(2)}% | ${target.toFixed(2)}% | ${diff >= 0 ? '+' : ''}${diff.toFixed(2)}%`);
    }
    // Shadow flags
